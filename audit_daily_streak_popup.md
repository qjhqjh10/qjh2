...
> 来源: d:/2/解包整理/03_界面UI/菜单 (原始 Unity JSON; 坐标=chain_rect.py v2 算法权威换算) — 生成 2026-08-23 16:54
...
| 元素 | 命中 |
|---|---|
| Daily Streak Popup | ✅ `scripts\daily_streak_popup.gd:3 ## 每日连胜弹窗 (原版 Daily Streak Popup [7107] 说明书):` |
...
| Streak Failed | ✅ `scripts\daily_streak_popup.gd:4 ##   Streak Failed: 'STREAK BROKEN' + 'Streak lost: 10' + 'Reset Streak' 按钮; scripts\daily_streak_` |
| Daily Streak Broken | ⚠️ 未命中 |
| Current Streak Lost count | ⚠️ 未命中 |
| Info | ✅ `scripts\achievements.gd:8 const TEX_CONTAINER := SPR + "UI_Deck_Information_submenu_Back.png"; scripts\base_event_popup.gd:40 # 红窗` |
...
| Streak Successful | ✅ `scripts\daily_streak_popup.gd:5 ##   Streak Successful: 'Current streak: 7' + Rewards Scroll View + 'More Rewards In 19h 23m'; scr` |
| Fill Line | ✅ `scripts\daily_streak_popup.gd:215 ## Streak Successful: 'Current streak: N' + Fill Line + 奖励滚动行 (Entry 380x516) + Timer; scripts\d` |
| Current Streak | ⚠️ 未命中 |
| Current Streak Value | ⚠️ 未命中 |
| Rewards Scroll View | ✅ `scripts\daily_reward_popup.gd:4 ## 原版: Rewards Scroll View [242,159 1705x806]; 每个 Entry = Day 标题 + Normal 奖励卡 295x381 + 进度 + Premi` |
...
| Timer Text | ⚠️ 未命中 |
| Header With Back Button | ✅ `scripts\daily_streak_popup.gd:6 ##   Header With Back Button: 'Daily Streak' + Back; scripts\daily_streak_popup.gd:98 # Header Wit` |
| Header Background | ⚠️ 未命中 |
| Window Title | ✅ `scripts\daily_streak_popup.gd:111 title.add_theme_font_size_override("font_size", 67)   # 原版 Window Title 67.55pt 白 (修正 36pt 金); s` |
| Header Background (1) | ⚠️ 未命中 |
| Header Back Button | ✅ `scripts\daily_streak_popup.gd:119 back.position = Vector2(-24, 24)   # 原版 Header Back Button [-24,24 168x111] (出屏 24 为原版设计); scrip` |
